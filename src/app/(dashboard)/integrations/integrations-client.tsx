...
export default function IntegrationsClient({ isLineConfigured, webhookUrl }: Props) {
  const [configured, setConfigured] = useState(isLineConfigured);
  ...
  async function handleSave() {
    if (!secret || !token) { setError("กรุณากรอกข้อมูลให้ครบ"); return; }
    ...
    try {
      ...
      setConfigured(true); setEditing(false); setSecret(""); setToken("");
  ...
  async function handleDisconnect() {
    ...
    setConfigured(false); setEditing(false);
  }
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-5">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5">
            ...
            <button onClick={copyWebhook} className="shrink-0 text-slate-400 hover:text-slate-600 transition-colors">
        ...
        {editing && (
          <div className="space-y-4 border-t border-slate-100 pt-4">
            <div>
              ...
              <div className="relative">
                <input type={showSecret ? "text" : "password"} value={secret}
                  onChange={(e) => setSecret(e.target.value)} placeholder="กรอก Channel Secret"
            ...
            <div>
              ...
              <div className="relative">
                <input type={showToken ? "text" : "password"} value={token}
                  onChange={(e) => setToken(e.target.value)} placeholder="กรอก Channel Access Token"
                  className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm pr-10 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { setEditing(false); setSecret(""); setToken(""); setError(""); }}
